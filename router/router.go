...
func InitBlogRoutes(r *gin.Engine, ac *ctrls.Controllers) {
	...
	blogGroup := r.Group("/api/v1")
	{
		// ================= 📝 文章前台接口 =================
		// 游客看文章列表（只看已发布）
		blogGroup.POST("/articles", ac.Article.GuestList)
		// 浏览单篇文章详情
		blogGroup.POST("/article/info", ac.Article.GetInfoById)

		// ================= ⚙️ 系统配置前台接口 =================
		// 前台获取单项配置（比如网站标题、ICP备案号）
		blogGroup.POST("/config/batch", ac.SystemConfig.GetBatchValues)
...
func InitAdminRoutes(r *gin.Engine, ac *ctrls.Controllers) {
	...
	{
		// ================= ⚙️ 系统配置项管理 =================
		adminGroup.POST("/config/create", ac.SystemConfig.CreateConfig) // 新增配置
		...
		adminGroup.POST("/config/list", ac.SystemConfig.GetConfigList)  // 获取配置分页列表
		adminGroup.POST("/config/info", ac.SystemConfig.GetConfigById)  // 通过ID反显配置详情
		...
		adminGroup.POST("/articles", ac.Article.AdminList)

		// ================= 👤 个人中心/密码管理 =================

		adminGroup.POST("/user/info", ac.User.GetInfoById) // 管理员登录后台后，获取自己的基本信息（如：头像、昵称）
		adminGroup.POST("/user/list", ac.User.List)        // 管理员登录后台后，获取自己的基本信息（如：头像、昵称）
